    ...
    def __init__(self, addr, buffer, prev=None, next=None, xrefs=[]):
        ...
        if self.opcode >= dis.HAVE_ARGUMENT:
            if not sys.version_info.major == 3:
                ...
                self.oparg = (buffer[1] | buffer[2] << 8)
        else:
            self.oparg = None
...
class BytecodeGraph():
    ...
    def apply_lineno(self):
        ...
        if not sys.version_info.major == 3:
            ...
            byte_increments = [c for c in self.code.co_lnotab[0::2]]
            line_increments = [c for c in self.code.co_lnotab[1::2]]
    ...
    def get_code(self, start=None):
        ...
        if sys.version_info.major == 3 and sys.version_info.minor >= 3:
            rvalue = types.CodeType(self.code.co_argcount,
